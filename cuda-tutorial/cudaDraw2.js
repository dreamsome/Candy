...
function cudaDrawProc2(p) {
    ...
    p.draw = function() {
        ...
        p.size(gridWidth + (arrayWidth+30)*3 + 20, gridHeight);
        p.background(255);
       ...
       for(var n=0; n<3; n++) {
        ...
        //draw a row 
        var row = p.threadIdx_y + p.blockDim_y * p.blockIdx_y;
        p.fill(230, 100, 134);
        ...
        //draw a col
        var col = p.threadIdx_x + p.blockDim_x * p.blockIdx_x;
        p.fill(230, 100, 134);
...
function changeAttributes2(blockIdx_x, blockIdx_y,
                           threadIdx_x, threadIdx_y, i) {
    var p = Processing.getInstanceById("canvas2");
